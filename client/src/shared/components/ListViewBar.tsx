    ...
    return (
        <div className="flex flex-row gap-1">
            {views.map((view) => (
                <div
                    key={view.type}
                    className="rounded-lg border border-primary-100 p-2"
                    onClick={() => handleListViewChange(view.type as ListView)}
                >
                    <img src={view.image} className={`h-4 w-4 ${listView !== view.type ? 'opacity-20' : ''}`} />
                </div>
            ))}
        </div>
    );
