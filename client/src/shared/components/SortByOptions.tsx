...
const SortByOptions = <T extends string>({
  ...
  const handleSortOptionChange = (event: React.ChangeEvent<HTMLSelectElement>) => { setSelectedSortOption(event.target.value as T); onSortOptionChange(event.target.value as T); };
  const handleOrderOptionChange: MouseEventHandler<HTMLImageElement> = () => { const newOrderOption = selectedOrderOption === 'asc' ? 'desc' : 'asc'; setSelectedOrderOption(newOrderOption); onOrderOptionChange(newOrderOption); };
  ...
  return (
    <div className='flex flex-row items-center gap-2'>
      ...
      <select className='p-1 border border-primary-100 rounded-lg' value={selectedSortOption} onChange={handleSortOptionChange}>
        {sortOptions.map(option => (
          <option key={option} value={option}>{option}</option>
        ))}
      </select>
      <img className='p-1 w-8 h-8 border border-primary-100 rounded-lg' src={selectedOrderOption === 'asc' ? DescOrderImage : AscOrderImage} onClick={handleOrderOptionChange}/>
    </div>
  );
};
